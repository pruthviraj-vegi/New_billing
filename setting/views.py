...
def shop_details_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        shop.delete()
        messages.success(request, f'Shop details for "{shop_name}" deleted successfully!')
        return redirect('setting:shop_details_list')
...
def report_config_delete(request, pk):
    ...
    if request.method == 'POST':
        config_name = f"{config.get_report_type_display()} - {config.get_paper_size_display()}"
        config.delete()
        messages.success(request, f'Report configuration "{config_name}" deleted successfully!')
        return redirect('setting:report_config_list')
...
def quick_report_settings(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            config = ReportConfiguration.get_default_config(ReportConfiguration.ReportType.INVOICE)
        ...
        config = ReportConfiguration.get_default_config(ReportConfiguration.ReportType.INVOICE)
        form = QuickReportConfigForm(initial={
...
def shop_settings_dashboard(request):
    ...
    configs = ReportConfiguration.objects.filter(is_active=True).order_by('-is_default', '-created_at')
